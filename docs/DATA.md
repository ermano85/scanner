...
KO, JNJ. Not a parser bug and not symbol-specific: a whole-market hole in the most
recently completed session. Reproduced under both `range=5d` and `range=1mo`.
...
- The data-quality gate compares the newest stored bar date against
  `calendar.last_completed_session()` and **fails the run** when they differ, rather than
  scanning stale data quietly.
- The nightly job refetches a `nightly_lookback_days` window rather than a single bar, so
  when the vendor backfills the gap it is picked up automatically.

This is the single best argument for moving to a paid vendor. It is also exactly why
`Provider` exists.
